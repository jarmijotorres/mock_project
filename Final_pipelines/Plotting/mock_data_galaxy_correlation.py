...
rp_LOWZ = wp_LOWZ[:,0] 
wp_rp = wp_LOWZ[:,1] / rp_LOWZ
wp_rp_err = wp_LOWZ[:,2] / rp_LOWZ

Lbox=['768','1536']
L_style = ['-','--']
f,ax = plt.subplots(2,1,figsize=(6,6.5),sharex=True,gridspec_kw={'height_ratios':[3,1.3]})
ax[0].errorbar(rp_LOWZ,wp_rp,yerr=3*wp_rp_err,fmt='ko',label = r'CODEX $0.2<z<0.4$')
ax[1].errorbar(rp,np.zeros_like(rp),yerr=3*wp_rp_err/wp_rp,fmt='ko')
lines = []
...
for L,li in zip(Lbox,L_style):
    wp_GR = np.loadtxt('/cosma7/data/dp004/dc-armi2/HOD_mocks/observables/clustering/JK125_wp_galaxies_GR_z0.3_L'+L+'_HOD_13.102_13.077_13.117_0.150_1.011_rp0.5_50_13rpbins.dat')
    wp_F5 = np.loadtxt('/cosma7/data/dp004/dc-armi2/HOD_mocks/observables/clustering/JK125_wp_galaxies_F5_z0.3_L'+L+'_HOD_13.161_13.982_13.580_0.111_0.991_rp0.5_50_13rpbins.dat')

    rp = wp_GR[:,0]
    wp_rp_GR = wp_GR[:,1] / rp
    wp_rp_GR_err = wp_GR[:,3] / rp
    #
    wp_rp_F5 = wp_F5[:,1] / rp
    wp_rp_F5_err = wp_F5[:,3] / rp

    wp_F5_residual = unumpy.uarray(wp_F5[:,1],std_devs=wp_F5[:,3])/ wp_LOWZ[:,1]
    wp_GR_residual = unumpy.uarray(wp_GR[:,1],std_devs=wp_GR[:,3])/ wp_LOWZ[:,1]
    ...
    l0,= ax[0].plot(rp,wp_rp_GR,'r',linestyle=li,linewidth=2.0)
    l1,= ax[0].plot(rp,wp_rp_F5,'b',linestyle=li,linewidth=2.0)
...
#
    ax[1].plot(rp,GR_ratio-1,'r',linestyle=li,linewidth=2.0)
    ax[1].plot(rp,F5_ratio-1,'b',linestyle=li,linewidth=2.0)
...
#plt.savefig('/cosma7/data/dp004/dc-armi2/HOD_mocks/plots/November2021/wp_rp_LOWZ_z0.2_0.4_MG_GADGET_GR_F5_L768_z0.3_rp_0.5_50_20rpbins.pdf',bbox_inches='tight')
plt.show()